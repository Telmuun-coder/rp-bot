...
app.get("/test", async (req, res) => {
  ...
  // nlp.classify('onoo orohgui bn').then((cls)=>{
  //
  //   console.log(cls)
  //   res.send(cls)
  //
  ...
  let cls = await nlp.classifyAsync('onoo orohgui bn')
  res.send(cls)
...
app.get("/profile", (req, res) => {
  ...
  if (mode && token) {
    if (token === config.verifyToken) {
      ...
      if (mode == "persistentmenu" || mode == "all") {
        Profile.setPersistentMenu()
        res.write(`<p>Persistent Menu</p>`);
      }
      if (mode == "getstarted" || mode == "all") {
        Profile.setGetStarted()
        res.write(`<p>Get Started</p>`);
      }
      if (mode == "greetings" || mode == "all") {
        Profile.setGreeting()
        Profile.
